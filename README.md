...
| **对话流卡片** | 总结卡头部 chip 含 Git 操作计数（本轮 git 调用次数，悬停看动词摘要） | 回合中间的已完成片段 = 轻量步骤卡（左竖线 + 淡纱）；回合最终回复 = 总结卡（「本轮完成」徽章 + 用时/步骤/工具/思考统计 chip + 顶部品牌蓝渐隐细线；中断回合变琥珀色「已中断」）。**卡片只在回合结束后出现**，流式期间一律平铺，流式输出不被卡片吞掉 |
